...
export const parentStudentMap = pgTable("parent_student_map", {
  id: uuid("id").primaryKey().defaultRandom(),
  parentId: uuid("parent_id").notNull().references(() => parents.id, { onDelete: "cascade" }),
  studentId: uuid("student_id").notNull().references(() => students.id, { onDelete: "cascade" }),
  createdAt: timestamp("created_at").notNull().defaultNow()
}, (table) => ({
  parentStudentIdx: index("parent_student_idx").on(table.parentId, table.studentId)
}));
...
export const teacherCourseMap = pgTable("teacher_course_map", {
  ...
  teacherCourseIdx: index("teacher_course_idx").on(table.teacherId, table.courseId)
}));
...
export const kpPrerequisites = pgTable("kp_prerequisites", {
  ...
  prerequisiteKpId: uuid("prerequisite_kp_id").notNull().references(() => knowledgePoint.id, { onDelete: "cascade" })
}, (table) => ({
...
export const sectionKpMap = pgTable("section_kp_map", {
  ...
  orderIdx: index("section_kp_order_idx").on(table.sectionId, table.orderIndex)
}));
...
export const questionMetadata = pgTable("question_metadata", {
  ...
  difficultyIdx: index("question_metadata_difficulty_idx").on(table.difficulty)
}));

export const kpExercises = pgTable("kp_exercises", {
  id: uuid("id").primaryKey().defaultRandom(),
  kpId: uuid("kp_id").notNull().references(() => knowledgePoint.id, { onDelete: "cascade" }),
  questionId: uuid("question_id").notNull().references(() => questionBank.id, { onDelete: "cascade" }),
  ...
  kpQuestionIdx: index("kp_exercises_kp_question_idx").on(table.kpId, table.questionId),
  difficultyIdx: index("kp_exercises_difficulty_idx").on(table.kpId, table.difficulty)
}));
...
export const studentAssignments = pgTable("student_assignments", {
  ...
  studentAssignmentIdx: index("student_assignments_student_assignment_idx").on(table.studentId, table.assignmentId),
  statusIdx: index("student_assignments_status_idx").on(table.status)
}));

export const studentAssignmentResults = pgTable("student_assignment_results", {
  id: uuid("id").primaryKey().defaultRandom(),
  studentAssignmentId: uuid("student_assignment_id").notNull().references(() => studentAssignments.id, { onDelete: "cascade" }),
  ...
  studentAssignmentIdx: index("student_assignment_results_idx").on(table.studentAssignmentId)
}));
...
export const questionAttempts = pgTable("question_attempts", {
  ...
  timeSpent: integer("time_spent").notNull(), // seconds
  kpId: uuid("kp_id").references(() => knowledgePoint.id, { onDelete: "set null" }),
  attemptTime: timestamp("attempt_time").notNull().defaultNow()
}, (table) => ({
  studentQuestionIdx: index("question_attempts_student_question_idx").on(table.studentId, table.questionId),
...
export const studentKpProgress = pgTable("student_kp_progress", {
  id: uuid("id").primaryKey().defaultRandom(),
  studentId: uuid("student_id").notNull().references(() => students.id, { onDelete: "cascade" }),
  kpId: uuid("kp_id").notNull().references(() => knowledgePoint.id, { onDelete: "cascade" }),
  masteryScore: integer("mastery_score").notNull(), // 0-100
  confidence: integer("confidence").notNull(), // 0-100, confidence in mastery score
  lastAttemptId: uuid("last_attempt_id").notNull().references(() => questionAttempts.id, { onDelete: "cascade" }),
  lastUpdated: timestamp("last_updated").notNull().defaultNow()
}, (table) => ({
  studentKpIdx: index("student_kp_progress_student_kp_idx").on(table.studentId, table.kpId),
  masteryIdx: index("student_kp_progress_mastery_idx").on(table.studentId, table.masteryScore)
}));

export const studentKpHistory = pgTable("student_kp_history", {
  id: uuid("id").primaryKey().defaultRandom(),
  studentId: uuid("student_id").notNull().references(() => students.id, { onDelete: "cascade" }),
  kpId: uuid("kp_id").notNull().references(() => knowledgePoint.id, { onDelete: "cascade" }),
  ...
  studentKpIdx: index("student_kp_history_student_kp_idx").on(table.studentId, table.kpId),
...
export const studentMastery = pgTable("student_mastery", {
  ...
  studentCourseIdx: index("student_mastery_student_course_idx").on(table.studentId, table.courseId)
}));
...
export const timeOnTask = pgTable("time_on_task", {
  id: uuid("id").primaryKey().defaultRandom(),
  studentId: uuid("student_id").notNull().references(() => students.id, { onDelete: "cascade" }),
  kpId: uuid("kp_id").references(() => knowledgePoint.id, { onDelete: "set null" }),
  ...
  studentKpIdx: index("time_on_task_student_kp_idx").on(table.studentId, table.kpId),
  studentSectionIdx: index("time_on_task_student_section_idx").on(table.studentId, table.sectionId)
}));
...
export const classes = pgTable("classes", {
  ...
  homeroomTeacherIdx: index("classes_homeroom_teacher_idx").on(table.homeroomTeacherId)
}));

export const classEnrollment = pgTable("class_enrollment", {
  id: uuid("id").primaryKey().defaultRandom(),
  ...
  classStudentIdx: index("class_enrollment_class_student_idx").on(table.classId, table.studentId),
  statusIdx: index("class_enrollment_status_idx").on(table.status)
}));

export const teacherClassMap = pgTable("teacher_class_map", {
  id: uuid("id").primaryKey().defaultRandom(),
  teacherId: uuid("teacher_id").notNull().references(() => teachers.id, { onDelete: "cascade" }),
  classId: uuid("class_id").notNull().references(() => classes.id, { onDelete: "cascade" }),
...
export const learningPathItems = pgTable("learning_path_items", {
  id: uuid("id").primaryKey().defaultRandom(),
  learningPathId: uuid("learning_path_id").notNull().references(() => learningPath.id, { onDelete: "cascade" }),
  ...
  learningPathIdx: index("learning_path_items_path_idx").on(table.learningPathId),
  orderIdx: index("learning_path_items_order_idx").on(table.learningPathId, table.orderIndex)
}));
...
export const recommendationEvents = pgTable("recommendation_events", {
  ...
  studentId: uuid("student_id").notNull().references(() => students.id, { onDelete: "cascade" }),
  kpId: uuid("kp_id").references(() => knowledgePoint.id, { onDelete: "set null" }),
  assignmentId: uuid("assignment_id").references(() => studentAssignments.id, { onDelete: "set null" }),
  sectionId: uuid("section_id").references(() => sections.id, { onDelete: "set null" }),
  recommendationType: varchar("recommendation_type", { length: 20 }).notNull(), // 'practice', 'review', 'advance'
  ...
  typeIdx: index("recommendation_events_type_idx").on(table.recommendationType),
  recommendedAtIdx: index("recommendation_events_recommended_at_idx").on(table.recommendedAt)
}));
