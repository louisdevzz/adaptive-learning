...
export class PrerequisiteService {
   ...
   */
  async getAllPrerequisites(kpId: string, visited = new Set<string>()): Promise<string[]> {
    if (visited.has(kpId)) {
      return [];
    }
    ...
    for (const { prerequisiteKpId } of prereqs) {
      ...
      const nestedPrereqs = await this.getAllPrerequisites(prerequisiteKpId, visited);
  ...
  async hasCompletedPrerequisites(
    studentId: string,
    kpId: string,
    getMasteryScore: (studentId: string, kpId: string) => Promise<number | null>,
  ): Promise<boolean> {
    const prereqs = await this.getAllPrerequisites(kpId);
